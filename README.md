...
| **Proactive bug fixes (0.2.8)** | (a) Cold-start path (index not ready) now calls the proactive enhancer hook instead of returning early — the user-reported case finally works as designed. (b) `filename_extend_should_fire` rewritten to trust `decision.intent` exclusively, removing a keyword-phrase fallback that was the third Principle-1 lapse in the project. (c) New firing condition: even on non-empty cascade results, fire if NONE of the basenames contain the lookup token | The user caught both regressions within hours of 0.2.7 ship. The cold-start hook means proactive actually runs on cold-start filename lookups against un-indexed dirs (the case it was designed for). The keyword-phrase removal is recorded in the [`PRINCIPLES.md`](docs/PRINCIPLES.md) receipts table with the prior three lapses. |
| **Proactive 3-case gate (0.2.9)** | `filename_extend_should_fire` now fires on (1) `intent in (filename,mixed)`, (2) `primary_token` non-empty even on semantic intent, OR (3) zero results + query contains identifier-shape token (digits/dashes/CamelCase). Token-shape detection is morphology, not keyword enumeration | 0.2.8's strict `intent ∈ {filename, mixed}` rejected the LLM-unreachable case where the rule-based fallback returned `intent=lexical, primary_token=""` even when the query had clear identifier shape. Case 3 catches that without enumerating trigger phrases. |
| **Proactive root-cause fix (0.2.10)** | Two real mechanism bugs found via end-to-end measurement: (a) per-dir `find` budget was divided by N instead of given full budget — `find` was killed at 133 ms before it could yield output (~161 ms typical); (b) defaults bumped `DEFAULT_TOTAL_BUDGET_MS=2000`, `filename_extend.individual_budget_ms=1500`. Plus gate simplification: pure results-based, no intent / token-shape gating | The 0.2.9 token-shape gating was a fourth Principle-1 lapse the user caught immediately. 0.2.10 ships the cleanest realisation: gate = "did cascade fail" (results-based only); mechanism = "extract token + parallel `find`, return None if no token". End-to-end verified against the user's real home dirs before tagging — 1093 ms wall clock to surface 4 actual matches. |
| **`recovery_progress_hint` enhancer + `ProactiveContext` (0.2.11)** | Second built-in proactive enhancer: when a content-search query hits an in-progress recovery worker (semantic intent + 0 hits + recovery active), surface live progress + ETA + next-step commands. Plus `ProactiveContext` infrastructure (`recovery_state` pre-fetched on main thread to avoid sqlite cross-thread issues) | Closes the gap 0.2.7-0.2.10 left: `filename_extend` covered "where is my file?", but content queries against an un-indexed dir got bare "No matches yet" with no ETA. Now they get the live re-embed progress + clear retry signal. Future graph-prior folder inference (designed in [`docs/plans/2026-05-05-graph-prior-folder-inference.md`](docs/plans/2026-05-05-graph-prior-folder-inference.md)) builds on the same `ProactiveContext` contract. |
| **`filename_extend` morphology fallback (0.2.12)** | Gate now extracts a candidate token via `_filename_token` + identifier-shape classifier when LLM is unreachable and `decision.primary_token` is empty. Cascade noise (`Project.toml` / `Manifest.toml` matching the user's token as a UUID substring) no longer silences proactive | The 0.2.10-0.2.11 gate gave up when LLM was offline, leaving the user's Desktop scenario broken. 0.2.12 restores the morphology fallback (digits / dashes / mixed-case classifier) so when the cascade returns junk, proactive still gets to try `~/Downloads`. End-to-end verified before tagging. Plus new plan doc on [conversational session state](docs/plans/2026-05-05-conversational-session-state.md). |

Full release notes: [`0.2.0`](docs/skylakegrep-0.2.0.md) · [`0.2.1`](docs/skylakegrep-0.2.1.md) · [`0.2.2`](docs/skylakegrep-0.2.2.md) · [`0.2.3`](docs/skylakegrep-0.2.3.md) · [`0.2.4`](docs/skylakegrep-0.2.4.md) · [`0.2.5`](docs/skylakegrep-0.2.5.md) · [`0.2.6`](docs/skylakegrep-0.2.6.md) · [`0.2.7`](docs/skylakegrep-0.2.7.md) · [`0.2.8`](docs/skylakegrep-0.2.8.md) · [`0.2.9`](docs/skylakegrep-0.2.9.md) · [`0.2.10`](docs/skylakegrep-0.2.10.md) · [`0.2.11`](docs/skylakegrep-0.2.11.md) · [`0.2.12`](docs/skylakegrep-0.2.12.md)
