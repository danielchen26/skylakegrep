...
def _path_to_module(path: Path, root: Path) -> tuple[str | None, str | None]:
    ...
    A file ``Rust workspace/crates/foo/src/bar/baz.rs`` resolves to crate=foo,
    ...
    # app/src/<...>.rs (Rust workspace's binary crate-ish layout)
...
def main() -> None:
    ...
    print(f"  scanned Rust workspace in {t_idx:.2f}s; {len(indeg)} files have non-zero in-degree")
    if indeg:
