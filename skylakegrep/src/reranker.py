...
``skygrep`` install stays light. Callers ask for the singleton via
...
DEFAULT_RERANK_MODEL = os.environ.get(
    "SKYGREP_RERANK_MODEL", "mixedbread-ai/mxbai-rerank-large-v2"
)
DEFAULT_RERANK_POOL = int(os.environ.get("SKYGREP_RERANK_POOL", "50"))
...
# faster than CPU. Override with ``SKYGREP_RERANK_DEVICE=cpu/mps/cuda``.
DEFAULT_RERANK_DEVICE = os.environ.get("SKYGREP_RERANK_DEVICE", "auto").lower()

# ``SKYGREP_RERANK_QUANTIZE=int8`` applies torch dynamic int8 quantisation on
# CPU. This pays off on x86_64 with VNNI but is roughly neutral on Apple
# Silicon CPU — MPS via ``SKYGREP_RERANK_DEVICE`` is the bigger lever there.
# Off by default; opt in for x86_64 deployments.
DEFAULT_RERANK_QUANTIZE = os.environ.get("SKYGREP_RERANK_QUANTIZE", "fp32").lower()
    ...
    try:
        ...
        if not _warned_missing:
            logger.warning(
                ...
                "rerank. Install with: pip install 'skylakegrep[rerank]'"
            )
