...
# changed. Tunable via ``SKYGREP_AUTO_REFRESH_THROTTLE_SECONDS`` (seconds).
...
def spawn_background_index(root: Path, db_path: Path, *, force: bool = False) -> int | None:
    """Launch ``skygrep index <root>`` as a detached background process.
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "skylakegrep.src.cli", "index", str(root)],
        cwd=str(root),
        env=env,
...
def _refresh_throttle_from_env() -> float:
    raw = os.environ.get("SKYGREP_AUTO_REFRESH_THROTTLE_SECONDS")
    if not raw:
