# skylakegrep 0.14.0 — release notes
...
```
$ skygrep "where is config file" -m 5

╭─ skylakegrep/src/config.py                            py   1.000
│ size: 5.3 KB    modified: 2026-05-04    type: py
╰────────────────────────────────────────────────────────────────

╭─ skylakegrep/src/integrations.py:87-109           python   0.609
│ symbol: register
│
...
╭─ skylakegrep/src/storage.py:292-328               python   0.608
...
```
$ skygrep "how does the cascade decide which path to take" -m 3

╭─ skylakegrep/src/storage.py:822-...                python   N.NN  ← cascade dominant
│ ...
╰─────
...
even a fresh repo answers `skygrep "where is README"` correctly
without needing to wait for the embedder.

## Files changed

  - `skylakegrep/src/intent.py` (new) — `classify_intent()` and
    `merge_results()`. ~140 lines.
  - `skylakegrep/src/cli.py` — filename + lexical shortcut blocks
  ...
  - `docs/skylakegrep-0.14.0.md` (this file).
...
```
skygrep "where is X file" --no-rg-shortcut --no-cascade
```
...
## Honest accuracy gates
  ...
  - **CLI smoke (filename intent)**: `skygrep "where is config file"`
    in the skylakegrep repo returns `config.py` first, then 4
    cascade chunks discussing config loading. Both tiers
    contribute, intent="filename" promotes the filename match
    to top.
  - **CLI smoke (semantic intent)**: `skygrep "how does the auth
    token get refreshed"` in a code repo runs cascade
    unaffected, ranks cascade chunks first, no irrelevant
    filename clutter.
  - **CLI smoke (downloads, no-index dir)**: `skygrep "where is
