# skylakegrep 0.5.0 — release notes
...
skygrep "how does the assistant call a language model?"
```
...
Behind the scenes, in priority order:
  ...
  4. **NEW (L4):** a file-export PageRank graph runs once per
     ...
     better leaves.
  5. **NEW (L3):** opt-in `skygrep enrich` runs a one-time background
...
## CLI surface changes
  ...
  - `skygrep enrich [PATH] [--max N] [--batch B]` — new top-level
    command. Resumable: Ctrl+C and re-run picks up where it stopped.
    Off by default at search time; export
    `SKYGREP_AUTO_ENRICH=yes` to have search auto-spawn enrichment in
    the background after a ready-index query.
  - `skygrep doctor` adds an `Enriched chunks` row showing
    `done / total (NN%)` so users know how far the L3 pass has
    progressed.
  - `skygrep stats` adds the same `Enriched` line.
...
```
  ...
  - `skygrep search` defaults are unchanged. New scoring kwargs
...
### L2 — Symbol-aware indexing (P5-SYM)
  ...
  - New `skylakegrep/src/storage.py::SYMBOL_WEIGHT` (default
    `0.10`, env `SKYGREP_SYMBOL_WEIGHT`).
  ...
  - New `skylakegrep/src/indexer.py::extract_file_symbols` —
...
### L3 — doc2query chunk enrichment (P6-D2Q)
  ...
  - New `skylakegrep/src/enrich.py::enrich_pending_chunks` —
  ...
  - New `skygrep enrich` CLI subcommand.
  - Optional background spawn under `SKYGREP_AUTO_ENRICH=yes` after
...
### L4 — File-export PageRank tiebreaker (P7-PR)
  ...
  - New `skylakegrep/src/code_graph.py` — regex-parses Rust /
  ...
  - New `storage._apply_graph_tiebreak`, called from
    ...
    `SKYGREP_TIEBREAK_EPS`). The tiebreaker bump is bounded by
    `GRAPH_TIEBREAK_WEIGHT` (default `0.005`,
    `SKYGREP_GRAPH_TIEBREAK_WEIGHT`); the safety property
...
## Performance expectations
  ...
  - Warm query with full enrichment (after `skygrep enrich` has
