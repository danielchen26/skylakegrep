# skylakegrep 0.8.0 — release notes
...
languages: skygrep cuts the agent's tool-call count by 54 % and improves
...
isn't an artefact, it's a direct consequence of skygrep returning
...
move the needle on the total. Token-cost claims for skygrep should be
based on tool-call reduction multiplied by per-tool-call payload,
not on the agent's overall token usage.

The **wall-time signal is contaminated** by spawning 6 skygrep-on
...
## Files changed
  ...
  - `docs/skylakegrep-0.8.0.md` (new release notes).
...
## Honest framing — when to cite which number
  ...
  - **"skygrep saves on agent tool calls"**: cite the 0.8.0 e2e
    benchmark (54 % reduction, 6 tasks, real Claude Code agent
    loop).
  - **"skygrep saves on token volume"**: cite the simulated
  ...
  - **"skygrep helps recall on real questions"**: cite the v0.7.0
