# skylakegrep 0.12.0 — release notes

A smart-routing release. skygrep now detects lexical-friendly queries
and short-circuits to ripgrep internally, returning in ~50 ms — so
calling `skygrep` is no longer ever a tax over `rg` for the easy cases.
For vocabulary-mismatch queries the existing semantic cascade runs
unchanged.

## Headline

`skygrep` becomes a **smart code-search router**, not just a semantic
search tool:

  - Lexical-friendly query (e.g. `skygrep "config defaults"`) →
    ripgrep short-circuit, ~50 ms.
  - Vocabulary-mismatch query (e.g. `skygrep "auth token refresh"`) →
    confidence-gated semantic cascade, sub-second to ~3 s.

The agent doesn't have to decide which path applies. skygrep auto-routes.
...
### Updated `skygrep setup` snippet

The markdown snippet `skygrep setup` writes into agent rules files
(Claude Code, Codex, OpenCode, Gemini CLI, Cursor) is rewritten for
v0.12.0:

> For any code-search question, prefer `skygrep "<query>"` over `rg`.
> skygrep is a smart router: lexical-friendly queries auto-route to rg
> internally (~50 ms), vocabulary-mismatch queries run the semantic
> cascade. You don't have to decide which path applies.

Old snippets continue to work; running `skygrep setup --uninstall`
followed by `skygrep setup` will swap in the new wording.

## Files changed

  - `skylakegrep/src/auto_index.py` — new `lexical_shortcut()` and
    four tuning constants.
  - `skylakegrep/src/cli.py` — new `--rg-shortcut/--no-rg-shortcut`
    flag (default on), call site between empty-index check and
    embedder load.
  - `skylakegrep/src/integrations.py` — updated `SNIPPET_BODY` to
  ...
  - `tests/test_lexical_shortcut.py` (new) — 8 tests covering
    ...
    + result annotation.
  - `docs/skylakegrep-0.12.0.md` (this file).
...
benchmark dimension must stay at least as good as v0.11.0.
  ...
  - **CLI smoke test**: `skygrep "config defaults" -m 5` returns the
...
silently reporting `skygrep_hit_rate: 0/30` for every task because of
...
If you previously ran `skygrep setup`, run it again with `--uninstall`
then `skygrep setup` to refresh the snippet wording. Existing
