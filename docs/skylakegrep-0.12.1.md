# skylakegrep 0.12.1 — release notes
...
```
=== /Users/.../skylakegrep/skylakegrep/src/config.py:114-128 (score: 0.557) ===
[file: skylakegrep/src/config.py] [lang: python] [symbol: get_config]
...
<https://danielchen26.github.io/skylakegrep/> hero card to feel
intentional, while staying pure ANSI (no extra dependency).

## What's new

### `skylakegrep/src/render.py` (new)
...
honoured. `SKYGREP_FORCE_COLOR=1` forces colour on for piped output
(useful for `skygrep ... | less -R`).

### Path display: repo-relative when possible

When the result's path lives under the project root, the CLI now
shows it repo-relative (`skylakegrep/src/config.py`) instead of
absolute (`/Users/.../skylakegrep/skylakegrep/src/config.py`). Same
...
## Files changed
  ...
  - `skylakegrep/src/render.py` (new) — ~210 lines.
  - `skylakegrep/src/cli.py` — every result-render site (5) now
    delegates to `render_terminal_result` or `render_compact_source`.
  - `pyproject.toml`: 0.12.0 → 0.12.1.
  - `docs/skylakegrep-0.12.1.md` (this file).
...
## Compatibility
  ...
  - `skygrep ... | less` / `skygrep ... > file` / `skygrep ... | jq`:
...
```
skygrep "your query"            # auto-coloured in your terminal
NO_COLOR=1 skygrep "your query" # opt-out, plain text
skygrep "your query" --json     # machine output, unchanged from v0.12.0
```
