# skylakegrep 0.4.1 — release notes
...
In 0.4.0 the bare-form ``skygrep "<query>"`` would auto-index a fresh
...
without ``skygrep index . --reset``. That is not a productive experience.

## What changed

### Ripgrep fallback for the first query

When ``skygrep "<query>"`` runs against a project whose semantic index
isn't ready yet, the CLI:

  1. Spawns a detached background indexer (``skygrep index <root>`` in a
...
```
$ time skygrep "where is the <domain-y> neural network architecture defined?" --top 3 --no-content
...
same query again in a minute, or run `skygrep stats` to see progress.
```

### ``skygrep stats`` shows live progress

``skygrep stats`` (run from inside the project) shows the indexer's
running tally even while the indexer is mid-run:

```
DB:           /Users/.../skylakegrep/repos/repo-B-21aa34e5.db
...
No flags added or removed. The flow is fully backwards compatible.
  ...
  - ``SKYGREP_DB_PATH`` still flips auto-index off (curated indexes are
    not auto-mutated).
  - ``skygrep index .`` is still the explicit / synchronous index path
    when you want to wait for completion.

## What changed under the hood

  - ``skylakegrep/src/auto_index.py`` — added ``is_index_ready``,
  ...
  - ``skylakegrep/src/cli.py`` — ``search_cmd`` now branches on
