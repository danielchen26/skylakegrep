...
| 3 | <repo-D> cross-repo vs **real ripgrep 15.1.0**, pre-P0 | 8/16 vs 16/16 | **868.6×** | 1256.98× | `benchmarks/parity_vs_ripgrep.py --tasks benchmarks/cross_repo/<repo-D>.json` |
| 3a | <repo-D> cross-repo, **after P0** (nomic-embed-text + cross-encoder rerank) | 9/16 vs 16/16 | **860.4×** | 1239.87× | same script with `--rerank` (default on) |
| 3b | <repo-D> cross-repo, **after P1** (P0 + chunk path/symbol prefix + chunker dedup) | 10/16 vs 16/16 | **528×** | 650× | `--rerank --reuse-index` after re-indexing |
...
### P0–P2 ablation on <repo-D> (top-k = 10, pool = 50 unless noted)
...
| above + non-canonical-path penalty (`/blocklist/`, `_test.rs`, etc., × 0.5) | 14/16 | no measurable lift on <repo-D>'s task set; kept in code as a small principled guard |
...
indexed-chunk count (53 382 → 26 454 on <repo-D>) without losing recall, because
...
embedding. On <repo-D> this lifts mean recall to ~12/16 (best 13/16) but introduces
...
## 3 — Cross-repo on <repo-D> (real ripgrep)

**Source:** `benchmarks/parity_vs_ripgrep.py` + `benchmarks/cross_repo/<repo-D>.json`

To rule out the "tasks are tied to local-mgrep" critique, the same benchmark
is run against the [<repo-D>](https://github.com/<repo-D>dotdev/<repo-D>) terminal source
...
**Result (top-k = 10, on the <repo-D> Rust workspace, ripgrep 15.1.0):**
...
On a large unfamiliar Rust codebase (<repo-D>, 65+ crates, ~3.2k files, ~46M
...
The ~50% recall on <repo-D> is materially worse than the 30/30 on the
...
The 50% <repo-D> recall gap above is being addressed by the structured plan in
...
- **Hand-curated task sets.** Both the 30-task self-test and the 16-task
  <repo-D> set were authored by hand. Generalization to arbitrary repositories
