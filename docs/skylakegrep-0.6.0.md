# skylakegrep 0.6.0 — release notes
...
skygrep "..."
skygrep "..."
skygrep "..."   # second+ query hits warm Ollama, no cold-load

skygrep doctor  # now reports HyDE model + keep-alive setting
```

`skygrep doctor` output gains two pieces of information:
...
## What changed under the hood
  ...
  - ``skylakegrep/src/config.py`` — adds ``DEFAULT_HYDE_MODEL``,
    ``DEFAULT_KEEP_ALIVE`` constants and surfaces ``hyde_model``,
    ``keep_alive`` from ``get_config()``.
  - ``skylakegrep/src/answerer.py`` — ``OllamaAnswerer`` now takes
...
## What did not change
  ...
  - L3 doc2query enrichment remains opt-in via ``skygrep enrich`` and
