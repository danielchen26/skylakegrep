# local-mgrep 0.15.0 — release notes
...
```
$ mgrep "where is eb1b file?" -m 5 --detail full
...
### `local_mgrep/src/llm_router.py` (new) — generic intent
...
- Default model: `qwen2.5:3b` (overridable via `MGREP_LLM_ROUTER_MODEL`).
- Hard timeout: 500 ms (overridable via `MGREP_LLM_ROUTER_TIMEOUT_SECONDS`).
- Confidence threshold for `skip_cascade=True`: 0.7 (overridable via
  `MGREP_LLM_ROUTER_MIN_CONFIDENCE`). Below threshold, cascade always
...
`mgrep search ... --no-llm-router` forces the rule-based path for
debugging or air-gapped use.

### `local_mgrep/src/binary_extract.py` (new) — PDF/docx content
...
## Files changed
  ...
  - `local_mgrep/src/llm_router.py` (new) — ~280 lines.
  - `local_mgrep/src/binary_extract.py` (new) — ~210 lines.
  - `local_mgrep/src/cli.py` — wires the router decision through
    the filename / lexical / cascade dispatchers; new flags
    `--llm-router/--no-llm-router`, `--detail`, `--ocr`.
  - `local_mgrep/src/render.py` — detail-level rendering branches,
    lazy binary content extraction for filename matches in `full`
    mode.
  - `pyproject.toml`: 0.14.0 → 0.15.0; added `pypdf>=3.0`,
    `python-docx>=1.0`.
  - `docs/local-mgrep-0.15.0.md` (this file).
...
benchmark's canonical answers:
  ...
  - **Miss 2** — query `"Where are Ollama and database environment
    variables read?"` expects `config.py`. mgrep returns
...
```
mgrep "where is README"               # filename + auto-extracted text
mgrep "how does the cascade decide"   # cascade only (LLM skips filename)
mgrep "config defaults"               # full hierarchical merge
mgrep "..." --detail full             # extract PDF/docx content inline
mgrep "..." --ocr                     # tesseract OCR (slow, opt-in)
mgrep "..." --no-llm-router           # force rule-based fallback
NO_COLOR=1 mgrep "..."                # plain output, no ANSI
```
