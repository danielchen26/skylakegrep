# skylakegrep 0.3.1 — release notes

Documentation patch on top of [0.3.0](skylakegrep-0.3.0.md). No behaviour
...
- **README rewrite** — Overview now describes the four-stage pipeline
  ...
  `skygrep serve`. A new "three retrieval tiers" table replaces the
...
patch bump, the PyPI page for `skylakegrep` would keep displaying the
...
new README so `https://pypi.org/project/skylakegrep/` matches the GitHub
README.
