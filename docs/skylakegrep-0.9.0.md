# skylakegrep 0.9.0 — release notes

A measurement release. The 0.8.0 e2e benchmark with 6 easy single-shot
questions hinted that skygrep cuts an AI agent's tool-call count by ~54 %.
...
**Replacing rg-only with skygrep in Claude Code agent loops cuts tool
...
semantic query in the set, skygrep cut tool calls 25× (25 → 1) and ran
1 / 8 the wall time.
...
| Total tokens | 471 K | 528 K | +12 % (skygrep slightly more) |
...
pure efficiency win on dimensions other than the token bill:**
  ...
  - Agent quality: with skygrep returning the canonical file directly,
    the agent doesn't get distracted re-reading false-positive
    candidates. Strict label correctness improves +2 tasks.

The token-cost claim that the original cloud skygrep markets is real
...
loop the agent's own thinking dominates the bill. Don't expect skygrep
...
skygrep returned `app/src/editor/view/mod.rs` on the first call. The
...
We also publish where skygrep didn't help:

|  | rg-only | skygrep-on | Δ |
|---|:-:|:-:|:-:|
| Tool calls | 5 | 7 | +40 % (skygrep slightly more) |
| Tokens | 31 K | 44 K | +44 % |
...
scan was already efficient. The skygrep agent ran 4 skygrep calls to
explore neighbouring concepts (auth_manager, credentials, login_slide)
before answering — it wandered. Both got the right answer
(`app/src/auth/...`).

The pattern: skygrep helps most when query vocabulary doesn't match
...
## How the experiment was run
  ...
  - Two prompt conditions per question:
      - **rg-only**: explicitly forbade `skygrep`; allowed `rg`,
        `find`, `ls`, `head`, `cat`, `wc`, `Read`, `Grep`.
      - **skygrep-on**: instructed `skygrep` as primary tool; `Read`
...
## Files changed
  ...
  - `docs/skylakegrep-0.9.0.md` — these notes.
