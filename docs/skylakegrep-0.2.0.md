# skylakegrep 0.2.0 — capability guide
...
`skylakegrep` is a command-line tool that builds a SQLite vector index from a
...
| --- | --- |
| `skylakegrep/src/cli.py` | Click-based CLI entry points (`index`, `search`, `stats`, `watch`) and result rendering. |
| `skylakegrep/src/indexer.py` | File discovery, ignore-rule application, tree-sitter / fallback chunking. |
| `skylakegrep/src/embeddings.py` | HTTP client for the Ollama embeddings API (batch and single-text endpoints). |
| `skylakegrep/src/storage.py` | SQLite schema, vectorized cosine search, lexical reranker, diversification. |
| `skylakegrep/src/answerer.py` | HTTP client for the Ollama generation API used by `--answer` and `--agentic`. |
| `skylakegrep/src/config.py` | Environment-variable configuration. |

## Indexing

`skygrep index` performs five operations on the given path:
...
`skygrep watch` repeats the same logic on a polling interval (default 5 seconds)
until interrupted.
...
`skygrep search` runs the query through five stages:
...
| `SKYGREP_DB_PATH` | `~/.skylakegrep/index.db` | SQLite index location. |

For per-project indexes, set `SKYGREP_DB_PATH` to a project-local file before
invoking `skygrep`.
...
| Metric | skylakegrep | grep-agent baseline |
| --- | --- | --- |
