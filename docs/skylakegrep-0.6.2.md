# skylakegrep 0.6.2 — release notes

Polish release. No new retrieval architecture, no recall change. Three
quality-of-life improvements:

  - **Ollama preheat on every search.** ``skygrep`` now fires fire-and-
...
## What changed
  ...
  - ``skylakegrep/src/bootstrap.py`` — new ``preheat_models()``
    ...
    swallowed.
  - ``skylakegrep/src/cli.py`` — ``search_cmd`` calls
