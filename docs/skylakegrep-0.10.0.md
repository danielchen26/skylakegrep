# skylakegrep 0.10.0 — release notes
...
On 5 of 6 medium-difficulty single-turn tasks, skygrep finds the
canonical file in **1** tool call vs rg-only's 4-8.

| Bench | Tasks × conds | rg-only tools | skygrep tools | Δ tools | Δ tokens |
|---|:-:|:-:|:-:|:-:|:-:|
...
The multi-turn pattern shows skygrep's tool-call advantage compounds:
by T3 the rg agent is still re-hunting through `rg` / `Read` while
the skygrep agent has the answer in 1 call.
...
5 of 6 skygrep agents finished in **1 tool call**. Token total **−8 %**.
All 6 skygrep agents got correct answers (lenient label match);
...
|---|:-:|:-:|:-:|
| Total tokens (sum) | 658 631 | 700 600 | +6.5 % (skygrep slightly more) |
...
## What the data actually shows
  ...
  - **Tokens are noisy.** −5 % to −8 % on subsets where skygrep
    agents are decisive (1 tool call); +6 % on the noisier 0.9.0
    hard-task subset where skygrep agents wandered. Net: roughly
    flat. **Don't claim skygrep saves the LLM bill.**
  - **Quality slightly better with skygrep.** +2 strict, +3 lenient
    on the 20-task aggregate. skygrep solves the repo-A `<domain-y>_v6`
  ...
  - **Wall-time data is contaminated** by parallel Ollama
    ...
    single skygrep agent at a time and saw skygrep wall time
...
## Files changed
  ...
  - `docs/skylakegrep-0.10.0.md` (this file).
