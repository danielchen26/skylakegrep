# skylakegrep 0.13.0 — release notes
...
```
$ skygrep "where is eb1b file?" -m 5
...
```
$ skygrep "config defaults loaded from environment" -m 2

╭─ skylakegrep/src/config.py:114-128 ───────────────────  python   0.557
│ symbol: get_config
│
│ def get_config():
...
with ``SKYGREP_PYGMENTS_STYLE=<name>``). All 300+ Pygments lexers are
...
## Files changed
  ...
  - `skylakegrep/src/auto_index.py` — `filename_shortcut()`
    (~120 lines, plus tuning constants).
  - `skylakegrep/src/cli.py` — `--filename-shortcut/--no-filename-shortcut`
    flag, call site placed BEFORE the index-ready check so the
    filename tier works on un-indexed directories.
  - `skylakegrep/src/render.py` — full rewrite: card frames,
  ...
  - `pyproject.toml` — `pygments>=2.0` dep added; version
    0.12.1 → 0.13.0.
  - `docs/skylakegrep-0.13.0.md` (this file).
...
## Compatibility
  ...
  - Pipe / redirect (`skygrep ... | cat`, `skygrep ... > out.txt`):
...
## Honest accuracy gates
  ...
  - **Self-test 30-task semantic benchmark** — held at 30 / 30
    ...
    phrases).
  - **CLI smoke test (filename path)**: `skygrep "where is eb1b file?"`
    in `~/Downloads/` returns the 5 EB1B documents in 0.6 s.
    Same query under v0.12.1 returned poetry.lock / SVG / events.jsonl
    junk — a real bug fix.
  - **CLI smoke test (cascade path)**: `skygrep "config defaults loaded
...
```
skygrep "where is README"           # filename-lookup tier (~10 ms)
skygrep "auth login"                # lexical content tier (~50 ms)
skygrep "how does the cascade work" # semantic cascade tier
skygrep "..." --no-filename-shortcut --no-rg-shortcut # force cascade
NO_COLOR=1 skygrep "..."            # opt out of ANSI colour
```
